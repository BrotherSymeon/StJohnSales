		...
		save: function(where, callback) {
			...
			return new Promise(function(resolve, reject) {
				...
				if(where) {
					...
					var q = "UPDATE "+tableName+" SET "+ connection.escape(self.attributes)+" WHERE "+where;
					if(id) {
						self.set('id', id);
					}
					...
					if(self.has('id')) {
						var id = self.get('id');
						delete self.attributes.id;
						var q = "UPDATE "+tableName+" SET "+ connection.escape(self.attributes)+" WHERE id="+connection.escape(id);
						self.set('id', id);
						...
						// Create new record
						var q = "INSERT INTO "+tableName+" SET "+ connection.escape(self.attributes);
						connection.query(q, function(err, result) {
							if(callback){
								return callback(err, result, connection);
							}
