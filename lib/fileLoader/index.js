...
FileUploader.prototype.parseResults = function (sqlResult) {
  const retVal = {};
  if (!Array.isArray(sqlResult)) {
    var temp = [sqlResult];
    sqlResult = temp;
    log('converted non array into %o', sqlResult);
  }
  ...
  log('#parseResults result to parse %o ', sqlResult);
  const flatResults = sqlResult.reduce(function (acc, val) {
    return acc.concat(val);
  }, []);
  const errorResults = flatResults.filter(function (res) {
    return res.Level === 'Error';
  });
  const results = flatResults.filter(function (res) {
    return Object.keys(res).includes('fieldCount');
  });
  const nonStandardResults = flatResults.filter(function (res) {
    return !Object.keys(res).includes('fieldCount') && !Object.keys(res).includes('Level');
  });
  retVal.error = errorResults[0] || null;
  retVal.results = results || null;
  retVal.extras = nonStandardResults || null;
  retVal.getExtra = function (key) {
    let extra = retVal.extras.find(function (x) {
      return Object.keys(x).includes(key);
    });
    if (!!extra) {
      return extra[key];
    } else {
...
FileUploader.prototype._insertIntoTempTable = function (lines, e, cb) {
  ...
  promises.push(function (done) {
    ...
    self.connection.queryOptions(
      ...
      (err, result) => {
        var affectedRows = 0;
        result = self.parseResults(JSON.parse(JSON.stringify(result)));
        ...
        if (err) {
          self.errors.push(err);
          return done(err, result.results);
        }
        ...
        return done(null, result.results);
      }
    );
  });
