...
module.exports = (mySqlConnection) => {
  return new FileUploader(mySqlConnection);
};
...
var FileUploader = function (conn){
  ...
  //this._saveProcessDetail = this._saveProcessDetail.bind(this)
...
FileUploader.prototype._insertIntoTempTable = function(lines, e, cb) {
  ...
  rows.forEach((line, index) => {
    promises.push(function(done) {
      self.connection.queryOptions(self.config.tempInsertSQL, [[line]], (err, result) => {
        if (err) {
        ...
        e.emit(self.events.SAVEDDATALINE, {
          ...
          percentDone:  (((index / rows.length) * THIRD_STEP) * 100)
        });
  ...
  promises.push(function(done) {
    ...
    //run the Stored PROC
    self.connection.query( self.config.loadDataSQL, (err, result) => {
      if (err) return done(err);
